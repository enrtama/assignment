...
import { LineChart, ComposedChart, AreaChart, Area, Bar, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, Label } from 'recharts'
...
export default class Chart extends React.Component<Props,State> {
  ...
  */
  componentWillReceiveProps = (nextProps: Props) => {
    ...
    if (vehicle) {
      let newData = {}
      newData[parameters[0]] = parameters[0] === PARAMETERS.TIME ? getTimeKey(vehicle.time) : vehicle[parameters[0]] || 0
    ...
    switch (chartType) {
      case CHART_TYPE.LINE:
        return <LineChart width={600} height={300} data={dataChart} margin={{top: 5, right: 5, left: 5, bottom: 5}}>
                  <XAxis dataKey={param0} allowDuplicatedCategory={false}>
